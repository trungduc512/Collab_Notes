...
export const addCollaborator = async (req, res) => {
  try {
    ...
    if (doesDocumentExist.owner.toString() === doesCollaboratorExist.id)
      return res.status(400).json({
        message: `${collaboratorEmail} is the owner of this document`,
      });

    if (doesDocumentExist.collaborators.includes(doesCollaboratorExist.id))
      return res
        .status(400)
        .json({ message: `${collaboratorEmail} is already a collaborator` });

    doesDocumentExist.collaborators.push(doesCollaboratorExist.id);
